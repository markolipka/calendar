...
#' @param start_time start time. By default the currently started hour.
#' @param end_time a number representing the number of hours
#' after `start_time` (if class `POSIXct`) or a datetime object (of class `POSIXct`) or
#' the number of days including `start_time` (if class `Date`) or a date object (of class `Date`) when
#' the event ends. By default set to `1`, meaning 1 hour after `start_time` or 1 day all-day event.
#' @param format required if start_time and end_time are vectors and are not of
#' datetime format "%Y-%m-%d %H:%M", you can use `calendar::formats` object for convenience.
...
ic_event <- function(
  ...
  posixct <- "POSIXct"
  properties_core <- calendar::properties_core
  # inputs
  ...
  if (is.na(start_time) & is.na(end_time)) {
    stop("no value provided for start and end time.")
  }
  if (is.na(start_time)) {
    warning("no value provided for start time. Set to now.")
    start_time <- as.POSIXct(round.POSIXt(Sys.time(), units = "hours"))
  }
  if (is.na(end_time)) {
    warning("no value provided for end time. Set to default value (see manpage).")
    end_time <- 1
  }
  ...
  if (is.numeric(end_time) & lubridate::is.POSIXct(start_time)) {
    end_time <- start_time + lubridate::hours(end_time)
  ...
  if (lubridate::is.Date(start_time) | lubridate::is.Date(end_time)){
    message("start or end is of type 'Date' without a time component. 'All day event' will be created.")
    start_time_char <- ic_char_date(start_time)
    end_time_char <- ifelse(is.numeric(end_time),
                            ic_char_date(start_time + lubridate::days(end_time - 1)),
                            ic_char_date(end_time))
    properties_core$DTSTART <- "DTSTART;VALUE=DATE"
    properties_core$DTEND <- "DTEND;VALUE=DATE"
  } else {
  ...
  event <- paste(properties_core, event_values, sep = ":")
  if (more_properties) {
