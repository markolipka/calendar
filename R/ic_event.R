...
#' after `start_time` or a datetime object (of class `POSIXct`) when
...
ic_event <- function(
  ...
  if (is.na(start_time)) {
    warning("no value provided for start time")
    start_time <- as.POSIXct(round.POSIXt(Sys.time(), units = "hours"))
  }
  if (is.na(end_time)) {
  ...
  if (lubridate::is.Date(start_time)){
  ...
  if (lubridate::is.Date(end_time)){
    end_time_char <- ic_char_date(end_time)
